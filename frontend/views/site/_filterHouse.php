...
<div class="row">
    <?= $houseForm->field($searchModel, 'action_id', ['options' => ['class' => 'input-field marg-top col s10 offset-s1']])
                  ->dropDownList(ArrayHelper::map(Action::getAll(), 'id', 'title'), ['prompt' => 'Все дома']) ?>
    <?= $houseForm->field($searchModel, 'districtId', ['options' => ['class' => 'input-field marg-top col s10 offset-s1']])
                  ->dropDownList(ArrayHelper::map(District::getAll(), 'id', 'name'), ['prompt' => 'Все направления'])
                  ->label('Направление') ?>
